...
export class BuyerComponent extends React.Component<BuyerComponentProps, BuyComponentState> {
    ...
    private saintItem(saint: Saint, index: number) {
        return (
            <li key={index}>
                <input name="saintSelect" ref="saintSelect" id={'saint' + saint.id} type="radio" value={saint.id}
                       onChange={(e) => {
                           this.selectSaint(e)
                       }}/>
    ...
    public chosenBuyer(buyerName: string) {
        ...
        let onBuyClick = () => {
            this.props
                .dispatch(BuyerActions.buy(
                    this.props.match.params.coffeeGroupId,
                    this.props.buyer.buyer.id,
                    this.props.buyer.consumerIds)
                )
                .then(() => {
                    this.props.history.push('/groups/' + this.props.match.params.coffeeGroupId);
                });
